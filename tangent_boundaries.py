...
def operators_square(parameters, plot_potential = False, plot_mass = False, plot_mag_field = False):
    ...
    for x in range(Nx-1):
        indices_to_delete.append(get_index(x,0,1)) #bottom edge
        indices_to_delete.append(get_index(x+1,Ny-1,1)) #top edge
        
    for y in range(0,Ny-1): 
        indices_to_delete.append(get_index(0,y+1,1)) #left edge
        indices_to_delete.append(get_index(Nx-1,y,1)) #right edge
...
def generate_stadium(r,L,npoints=1000000):
    'Generates the set of points in the grid closest to an stadium with radii r1 and r2 and the angle of the normal vector.'
...
def operators_stadium(parameters, plot_potential = False, plot_mass = False, plot_mag_field = False,return_shape = False):
    ...
    edge_points, normal_angles = generate_stadium(r = Ny//2-1, L = Nx//2-Ny//2-1)
    edge_points = edge_points + np.array([[Nx//2]*(len(edge_points[1])),[Ny//2]*(len(edge_points[1]))])
    # the parameter that we need for the spin rotation is the projection of the boundary spin o the plane, so the normal plus pi/2.
    boundary_spin_projections = normal_angles + np.ones(len(normal_angles))*pi/2
